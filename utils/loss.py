...
class UnbiasedKnowledgeDistillationLoss(nn.Module):
    ...
    def forward(self, inputs, targets, mask=None):
        ...
        new_cl = inputs.shape[1] - targets.shape[1] #input이 17이고 target이 16이여야하는데
        # print(new_cl)
        ...
        # print(outputs_bkg.shape)
        # print(outputs_no_bgk.shape)
...
class visFODistill(nn.Module):
    def __init__(self):
        super().__init__()
        self.cos = nn.CosineSimilarity(dim=1)

    def forward(self, output, output_old, alpha):         
        cos = self.cos(output, output_old)
        loss = 1 - cos

        weighted_loss = (alpha * loss).mean()
        return weighted_loss

class clsFODistill(nn.Module):
    def __init__(self):
        super().__init__()
        self.cos = nn.CosineSimilarity(dim=1)
        
    def forward(self, output, output_old, beta, old_classes):
        output = output[:, :, :old_classes]
        output_old = output_old[:, :, :old_classes]

        loss = 1 - self.cos(output, output_old)

        weighted_loss = (beta * loss).mean()
        return weighted_loss
...
class BMloss(nn.Module):
    ...
    def forward(self, output, output_old, B_i, old_classes):
        output = output[:, old_classes:, :, :]
        
        B_i = B_i.unsqueeze(1)

        loss_new = 1 - (2 * torch.sum(B_i * output) / (torch.sum(B_i**2) + torch.sum(output**2)))

        B_t_complement = 1 - B_i  
        loss_old = 1 - (2 * torch.sum(B_t_complement * output_old) / (torch.sum(B_t_complement**2) + torch.sum(output_old**2)))
