...
def gather_data(seg_pred, gpu, rank, world_size, tmp_dir=None):
    """
    ...
    tmpprefix = '/home/nayoung/nayoung/segmenter/segm/temp'
    
    MAX_LEN = 512
    # 32 is whitespace
    dir_tensor = torch.full((MAX_LEN,), 32, dtype=torch.uint8, device=gpu)
    if rank == 0:
        tmpdir = tempfile.mkdtemp(prefix=tmpprefix)
        tmpdir = torch.tensor(
            bytearray(tmpdir.encode()), dtype=torch.uint8, device=gpu)
    ...
    """
    tmp_file = tmpdir / f"part_{rank}.pkl"
...
def compute_metrics(
    ...
    ret_metrics_mean = torch.zeros(3, dtype=float, device=gpu)
    if rank == 0:
        list_seg_pred = []
        list_seg_gt = []
